...
function normalize(arr){const min=Math.min(...arr),max=Math.max(...arr);return arr.map(v=>(v-min)/(max-min+1e-6));}
function cosineSim_norm(a,b){const na=normalize(a),nb=normalize(b);const dot=na.reduce((s,v,i)=>s+v*nb[i],0);const ma=Math.sqrt(na.reduce((s,v)=>s+v*v,0));const mb=Math.sqrt(nb.reduce((s,v)=>s+v*v,0));return ma===0||mb===0?0:dot/(ma*mb);}
const NPK_MAX={N:500,P:300,K:400};
function computeSmartNPK(adc){
  if(!adc||adc.every(v=>v===0))return{N:0,P:0,K:0};
  const simN=cosineSim_norm(adc,REFS.nitrogen);
  const simP=cosineSim_norm(adc,REFS.phosphorus);
...
export default function App() {
  ...
  const _npk=computeSmartNPK(adc);
  const NPK=[{el:"N",val:_npk.N,max:NPK_MAX.N,col:"#4ade80"},{el:"P",val:_npk.P,max:NPK_MAX.P,col:"#fb923c"},{el:"K",val:_npk.K,max:NPK_MAX.K,col:"#c084fc"}];
